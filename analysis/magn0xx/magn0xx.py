...
#------------------------------
print('')
print('[info] starting magn0xx.py | dumb full-grid-search fitting of Zeeman broadening')
...
ff = magnotron_loadff('ff0xx.p')  # loads the filling factor combinations
...
b_combinations = combinations(range(nmf),2)
b_combinations = list(b_combinations)
n_b = len(b_combinations)
nff = len(ff[0,:])  # number of filling factor combinations
chi = np.zeros([nff,n_b]) # init the chisquare array
scale = np.zeros([nff,n_b])   # init scaling factor array
modBarr = np.zeros([nff,n_b]) # init <B> array
dof = len(wlsel)-6  # degree of freedom = number of wavelength points - 2 (filling factors) -1 (scaling factor) -2 (m.f) -1 (there's always -1 anyway that you don't know why)
dofl = float(dof)   # converts from int to flt
b_counter = 0
for imf1,imf2 in combinations(range(nmf),2):
    #print(imf1,imf2)
    ...
    # for each combination: (1) compute the corresponding magnetic spectrum, (2) find the optimal scaling factor, (3) compute the chi-square then
        sc,ch = magnotron_findscale(0.01*(sy[0,:] * ff[0,ii] + sy[imf1,:] * ff[1,ii] + sy[imf2,:] * ff[2,ii]),y,scl,inputpar.snr[iobs])
        scale[ii,b_counter] = sc  # scale
        chi[ii,b_counter] = ch    # chisquare
        modBarr[ii,b_counter] = 0.01 * (ff[1,ii]*mf[imf1] + ff[2,ii]*mf[imf2])  # <B>
    b_counter += 1

modB,wbest = magnotron_bstat(modBarr,chi,dof)
minchi = np.amin(chi)   # minimum chi-square value

spec = (0.01*(synth[0,:] * ff[0,wbest[0]] + synth[b_combinations[wbest[1]][0],:] * ff[1,wbest[0]] + synth[b_combinations[wbest[1]][1],:] * ff[2,wbest[0]])) * scale[wbest] +1. - scale[wbest]   # spectrum for the solution
...
#logf = open('results/'+file_prefix+'magn0xx.log',"a")
